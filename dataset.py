...
DATASETS = ['FEMNIST', 'RareGroupRotatedMNIST', 'TinyImagenet', 'WILDSCamelyon', 'MIMICCXR']
...
class MIMICCXR(MultipleDomainDataset):
    """MIMIC-CXR dataset for multi-instance learning"""
    DEFAULT_METADATA_PATH = '/data/healthy-ml/gobi1/data/MIMIC-CXR-JPG/subpop_bench_meta/'
    
    def __init__(self, root, test_envs, hparams):
        super().__init__()
        # Always use the default path for this dataset
        self.root = self.DEFAULT_METADATA_PATH
        self.ctxt = hparams.get('context_length', 1)
        
        # Load metadata CSV
        meta_path = os.path.join(self.root, 'metadata_no_finding.csv')
        if not os.path.exists(meta_path):
            raise ValueError(f'Metadata file not found at {meta_path}')
        
        print(f"=> Loading MIMIC-CXR from: {self.root}")
        metadata = pd.read_csv(meta_path)
        
        # Group by study_id and split into train/val/test
        studies = metadata.groupby('study_id')
        study_ids = list(studies.groups.keys())
        np.random.seed(hparams['trial_seed'])
        np.random.shuffle(study_ids)
        
        n_studies = len(study_ids)
        train_ids = study_ids[:int(0.7 * n_studies)]
        val_ids = study_ids[int(0.7 * n_studies):int(0.85 * n_studies)]
        test_ids = study_ids[int(0.85 * n_studies):]
        ...
        # Create environments
        self.datasets = [MIMICDataset(self.root, train_ids, studies, transform, self.ctxt)]
        self.validation = [MIMICDataset(self.root, val_ids, studies, transform, self.ctxt)]
        self.holdout_test = [MIMICDataset(self.root, test_ids, studies, transform, self.ctxt)]
        
        # Create caches for ICRM
        self.train_cache_x, self.train_cache_y = self._create_cache(self.datasets[0])
        self.valid_cache_x, self.valid_cache_y = self._create_cache(self.validation[0])
        self.test_cache_x, self.test_cache_y = self._create_cache(self.holdout_test[0])
    ...
    def _create_cache(self, dataset):
        """Create simple cache for ICRM algorithm"""
        if len(dataset) > 0:
            # Get a few examples for the cache
            cache_size = min(5, len(dataset))
            x_samples = []
            y_samples = []
            
            # Collect samples
            for i in range(cache_size):
                x, y = dataset[i]  # Now unpacks correctly
                x_samples.append(x)
                y_samples.append(y)
            
            # Stack into proper format for ICRM
            x_cache = [torch.stack(x_samples).unsqueeze(0)]  # [1, samples, context, C, H, W]
            y_cache = [torch.stack(y_samples).unsqueeze(0)]  # [1, samples]
            return x_cache, y_cache
        else:
            # Empty cache
            return [torch.zeros(1, 1, self.ctxt, 3, 224, 224)], [torch.zeros(1, 1, dtype=torch.long)]


class MIMICDataset(torch.utils.data.Dataset):
    """Simple MIMIC-CXR dataset that returns tensors directly"""
    
    def __init__(self, root, study_ids, studies, transform, context_length):
        self.root = root
        self.transform = transform
        ...
        for study_id in study_ids:
            study = studies.get_group(study_id)
            self.metadata.append({
                'study_id': study_id,
                'meta': study,
                'label': study['label'].iloc[0]
            })
    ...
    def __getitem__(self, idx):
        ...
        study = self.metadata[idx]
        meta_df = study['meta']
        label = torch.tensor(study['label'])
        
        # Load images up to context_length
        images = []
        for _, row in meta_df.iterrows():
            ...
            try:
                ...
                # Use zeros for missing/corrupt images
                if images:
        ...
        # Pad if needed
        while len(images) < self.context_length:
            if images:
                images.append(torch.zeros_like(images[0]))
            else:
                images.append(torch.zeros(3, 224, 224))
        
        # Stack into tensor of shape [context_length, C, H, W]
