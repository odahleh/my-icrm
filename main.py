...
def log_memory():
    ...
    if torch.cuda.is_available():
        print(f"GPU: {torch.cuda.memory_allocated() / (1024 * 1024):.2f} MB / {torch.cuda.memory_reserved() / (1024 * 1024):.2f} MB")
...
if __name__ == "__main__":
    ...
    if args.wandb:
        ...
        wandb.init(
            project=args.project if args.project else f"{args.dataset}_{args.algorithm}",
            entity=args.user if args.user else None,
            name=args.run_name,
            config=wandb_config,
    ...
    for step in range(start_step, n_steps):
        ...
        if args.wandb:
            ...
            if step % args.print_freq == 0:
                memory_stats = {}
                memory_stats['ram_mb'] = psutil.Process().memory_info().rss / (1024 * 1024)
                if torch.cuda.is_available():
